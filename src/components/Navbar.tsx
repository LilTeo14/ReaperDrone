...
export default function Navbar({ lang, setLang }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full glass-panel border-b border-[#242a35]/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        
        {/* Brand Logo */}
        <a href="#" className="flex items-center space-x-3 group">
          <div className="relative w-10 h-10 overflow-hidden bg-black/50 border border-[#242a35] rounded-md flex items-center justify-center p-1">
            <Image 
              src="/logo.png" 
              alt="Reaper Defence" 
              width={36} 
        ...
        <div className="hidden md:flex items-center space-x-6">
          ...
          <button 
            ...
            className="flex items-center space-x-1.5 text-xs text-[#8a99ad] hover:text-[#ff6b00] border border-[#242a35] hover:border-[#ff6b00]/40 px-3 py-1.5 rounded-sm font-mono transition-all duration-300 bg-black/35"
          >
        ...
        <div className="md:hidden flex items-center space-x-4">
          <button 
            onClick={() => setLang(lang === 'en' ? 'es' : 'en')}
            className="flex items-center space-x-1 text-xs text-[#8a99ad] border border-[#242a35] px-2 py-1 rounded-sm font-mono bg-black/35"
          >
          ...
          <button
            ...
            className="text-[#8a99ad] hover:text-white p-1 rounded-sm border border-[#242a35] hover:border-[#ff6b00]/40 transition-colors"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="md:hidden border-t border-[#242a35]/60 bg-[#090b0e] px-4 pt-4 pb-6 space-y-3"
          >
            {navLinks.map((link) => (
              <a
                key={link.href}
                ...
                className="block px-3 py-2 text-[#8a99ad] hover:text-white hover:bg-[#12151a] rounded-sm font-mono text-sm tracking-wide transition-colors border-l-2 border-transparent hover:border-[#ff6b00]"
              >
                {link.label}
              </a>
            ))}
            <div className="pt-4 flex items-center justify-between border-t border-[#242a35]/40 px-3">
